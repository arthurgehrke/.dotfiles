...
export FZF_DEFAULT_COMMAND='rg --files --hidden --follow --no-ignore-vcs --color=always --glob "!{.git,.idea,.vscode,.sass-cache,node_modules,build,Library,Music,Movies,Applications,.DS_Store,.cache,.cargo,.npm,.local,.bin,.Trash,.pyenv,.rbenv,.redis-insight,.rustup,.stack,.julia,.matplotlib,.stack,Alfred,go,.config/nvim/undodir}"'
# export FZF_ALT_C_COMMAND='fd --follow --type d --exclude "Library/" --exclude "Music/"'
export FZF_ALT_C_COMMAND='rg --files --hidden --follow --no-ignore-vcs --color=always --glob "!{Library,Music,Movies}" | xargs -n1 dirname | sort -u'
